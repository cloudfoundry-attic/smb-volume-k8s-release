...
var javaNodeGoBuildPack = `apiVersion: build.pivotal.io/v1alpha1
kind: ClusterBuilder
metadata:
  name: default
spec:
...
metadata:
  ...
  annotations:
    build.pivotal.io/docker: {{.RegistryPrefix}}
...
var serviceAccountRegistrySecretYaml = `apiVersion: v1
kind: ServiceAccount
metadata:
 name: smb-broker-github-credentials-service-account
secrets:
 - name: smb-broker-github-credentials`
...
spec:
  tag: {{.DockerImageName}}
  serviceAccount: smb-broker-github-credentials-service-account
  cacheSize: "1.5Gi"
  builder:
    name: default
    kind: ClusterBuilder
